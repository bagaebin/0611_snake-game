...
- **수락 기준**:
  ...
  - 게임 규칙과 조작법이 담긴 인트로 오버레이가 기본으로 표시되며, "게임 시작" 버튼을 눌러야 본게임이 진행된다.
  - 인트로 오버레이는 기울기 조작, 좌우 화살표 보조 조작, 코인/충돌 규칙을 요약해 제공하고 게임 시작 후에는 사라진다.
  - iOS를 포함한 모바일 브라우저에서 자이로센서 권한 요청을 트리거하는 버튼이 제공된다.
  - UI는 #01C756 배경과 검정색 버튼 대비를 사용하며, HUD 카드 역시 동일 팔레트로 일관성을 유지한다.
  - 본게임이 시작되면 인트로 외의 하단 설명 텍스트는 숨겨져 플레이 화면만 남는다.
...
- **수락 기준**:
  ...
  - 16x16 지면은 배경보다 더 밝은 #29FFA4와 #A6FFE4가 번갈아 칠해진 체커 패턴으로 표현되어 캐릭터/코인과 대비된다.
  - 캐릭터 헤드는 #F23D9B, 몸통은 #30E5FF와 #0181FD가 번갈아 나타나 시각적으로 구분된다.
...
- **수락 기준**:
  ...
  - 데스크톱 환경에서는 좌우 화살표 키로 현재 진행 방향을 기준으로 회전할 수 있으며, 왼쪽 키는 시계 방향, 오른쪽 키는 반시계 방향으로 회전한다.
  - 격자 벽을 향해 이동 중일 때 입력 벡터에 포함된 미세한 평행 성분을 살려 벽을 따라 미끄러지며 코너에 걸리지 않는다.
...
- **수락 기준**:
  ...
  - 카메라의 아이소메트릭 축을 기준으로 기기를 기울였을 때 화면에서 더 낮아진 방향과 캐릭터 진행 방향이 정확히 일치한다.
...
- **수락 기준**:
  ...
  - 코인은 자체 발광 색상과 포인트 라이트를 사용해 주변 지면을 은은하게 비춘다.
...
- 3D 캔버스는 항상 뷰포트 안에서 잘리지 않도록 높이가 동적으로 제한된다.
